...
sequenceDiagram
    ...
    Dev->>Compose: docker-compose -f docker-compose-test.yml up
...
3. **Run Stage**: `docker-compose -f docker-compose-test.yml up` starts Vireo + PostgreSQL
...
- Entrypoint fail-fast on missing required variables

**Consolidation Analysis**:
- Requirements 2.1, 6.1, 6.2, 6.3, and 6.5 all test environment variable override for database configuration — combined into one property
- Requirements 2.2, 2.3, 7.1, and 7.2 all test environment variable override for security secrets — combined
- Requirement 2.4 is a general statement that encompasses all environment variable overrides — parent property
- Requirement 2.5 (logging) is distinct and remains separate
- Requirement 7.4 (fail-fast on missing vars) is a distinct entrypoint behavior
...
3. Entrypoint fail-fast validation
...
**Rationale**: Spring Boot's externalized configuration follows a precedence hierarchy where environment variables override properties files. This property ensures that all configuration can be injected at runtime via environment variables, which is essential for switching between H2 and PostgreSQL and for injecting secrets without baking them into the image.

**Test Strategy**: Generate random valid configuration values for various Spring Boot properties, set them as environment variables, start the application, and verify it uses the environment variable values rather than defaults.

### Property 2: Application Logging to Standard Streams

*For any* log message generated by the application at any log level (INFO, WARN, ERROR, DEBUG), the message SHALL be written to STDOUT or STDERR.

**Validates: Requirements 2.5**

**Rationale**: Developers rely on `docker-compose logs` and `docker logs` to view application output. By ensuring all application logs go to standard streams (rather than only to files), we enable visibility into application behavior during local testing.
...
### Property 3: Entrypoint Fail-Fast Validation

*For any* required environment variable (AUTH_SERVICE_URL, LOCAL_AUTHENTICATION, STOMP_DEBUG, APP_PATH) that is not set, the entrypoint script SHALL exit with code 1 and a descriptive error message to STDERR.

**Validates: Requirements 7.4**

**Rationale**: Fast failure with clear error messages prevents developers from debugging silent misconfigurations. If a required variable is missing, the container should fail immediately rather than starting in a broken state.

**Test Strategy**: For each required variable, start the container with that variable unset and verify the container exits with code 1 and the error message names the missing variable.
...
- Entrypoint fail-fast for any missing required variable
...
- Verify Maven build uses -Pproduction and -Dmaven.test.skip=true flags
- Verify NODE_ENV=production in Maven stage
- Verify gettext package installed for envsubst
- Verify wget package installed for health check
- Verify OCI image labels present (title, version, created, revision, source, vendor)
...
**Tag**: `Feature: jhu-docker-image, Property 1: For any Spring Boot configuration property that has a corresponding environment variable set, the application SHALL use the environment variable value instead of the default value from application.yml`

```java
@Property
@Tag("Feature: jhu-docker-image, Property 1")
void environmentVariableOverridesConfiguration(
    ...
    // Start application with env var, verify it uses envValue
}
```

**Property Test 2: Application Logging to Standard Streams**

**Tag**: `Feature: jhu-docker-image, Property 2: For any log message generated by the application at any log level, the message SHALL be written to STDOUT or STDERR`

```java
@Property
@Tag("Feature: jhu-docker-image, Property 2")
void applicationLogsToStandardStreams(
    @ForAll("logLevel") String level,
    @ForAll("logMessage") String message) {

    // Trigger log at level, capture stdout/stderr, verify message present
}
```
...
- Run with PostgreSQL: `docker-compose -f docker-compose-test.yml up --build`
